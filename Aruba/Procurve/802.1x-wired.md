...
To use ClearPass with fingerprinting and MAC Address Bypass (MAB), the switch must be configured to send 802.1x requests to the ClearPass server, and the port must be set for 802.1x.  <br>

## configure the switch for dynamic segmentation using the Clearpass server<br>
...
```
After entering the alias in global configuration mode, type "aaa" [enter], and the alias will output the aaa configuration.<br>
...
NOTE: This command must be run from global configuration mode. If you are in an interface configuration mode, they will fail.
